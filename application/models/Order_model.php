        ...
        foreach ($user_details as $row) {
            ...
            if (!empty($row['items'])) {
                ...
                if (isset($row['mobile']) && !empty($row['mobile']) && $row['mobile'] != "" && $row['mobile'] != " ") {
                    $tempRow['mobile'] =   '<div class="d-flex gap-1 align-items-center justify-content-between">' . $row['mobile'] . "<a href='https://api.whatsapp.com/send?phone=" . $row['mobile'] . "&text=Hello " . $row['items'][0]['uname'] . ", Your order with ID :" .  $row['id'] . " and is " .  $row['active_status'] . ". Please take a note of it. If you have further queries feel free to contact us. Thank you.' target='_blank' title='Send Whatsapp Notification' class='btn btn-success btn-xs rounded-1 py-1'><ion-icon class='align-bottom fs-3' name='logo-whatsapp'></ion-icon></a></div>";
                } else {
                    $tempRow['mobile'] = "";
                }
